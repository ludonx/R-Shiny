...
#observe(input$ActionValider_definition_Les_Genes,{
  ...
  if(nrow(globalInformation$modules$Catalogue$dataFrameRequete)>0){
    ...
    for(i in 1:length(listeCollection)){
      lc<-listeCollection[i]
      lc<-str_sub(lc,1,str_length(lc)-1)
      globalInformation$modules$Catalogue$dataFrameRequete[i,"id_collection"]<-listeCollection[i]
      globalInformation$modules$Catalogue$dataFrameRequete[i,"collection"]<-lc
      ...
      df<-dataFrameDefinition()[dataFrameDefinition()["name"]==lc ,"database"]
...
observe({
  ...
  uisRequetesSuite<-lapply(1:nbrMaxCollection,function(indiceCollection){
    ...
    observeEvent(input[[ActionAjouter_une_contrainte_Les_Genes]],{
      ...
      if(nrow(globalInformation$modules$Catalogue$dataFrameContrainte)>0){
        for(k in 1:nrow(globalInformation$modules$Catalogue$dataFrameContrainte)){
          if(
            ...
            &&
            (globalInformation$modules$Catalogue$dataFrameContrainte[k,"id_collection"]==lc)
             ){
      ...
      globalInformation$modules$Catalogue$dataFrameContrainte[i+1,"id_collection"] <- lc
      globalInformation$modules$Catalogue$dataFrameContrainte[i+1,"collection"]<-str_sub(lc,1,str_length(lc)-1)
    ...
    output[[uiMes_contraintes_Les_Genes]]<-renderUI({
      ...
      if(nrow(globalInformation$modules$Catalogue$dataFrameContrainte)>0){
        for(k in 1:nrow(globalInformation$modules$Catalogue$dataFrameContrainte)){
          if(globalInformation$modules$Catalogue$dataFrameContrainte[k,"id_collection"]==lc){
    ...
    output[[text_Config_ma_requete_resultat_Les_Genes]]<-renderText({
      ...
      nbrElemRequete<-globalInformation$modules$Catalogue$dataFrameRequete[globalInformation$modules$Catalogue$dataFrameRequete["id_collection"]==lc,"nbrElemRequete"]
      paste0("Nombre d'éléments trouvé : ", nbrElemRequete)
    })
    output[[uima_requete_json]]<-renderText({
      paste0(globalInformation$modules$Catalogue$dataFrameRequete[globalInformation$modules$Catalogue$dataFrameRequete["id_collection"]==lc,"requete"])
    })
      ...
      fluidPage(
          ...
          fluidRow(
                   textInput(inputId = textinput_name_data_frame_Les_Genes,
                             ...
                             value =paste0(str_sub(lc,1,str_length(lc)-1))
    ...
    observeEvent(input[[ActionSave_data_frame_requete_Les_Genes]],{
      ...
      globalInformation$modules$Catalogue$dataFrameRequete[globalInformation$modules$Catalogue$dataFrameRequete["id_collection"]==lc,"requete"]<-myQueryString
      ...
      globalInformation$modules$Catalogue$listSaveDataFrame[[dfName]]<-findQuery(db_lc,str_sub(lc,1,str_length(lc)-1),myQueryString)
      ...
      if(indiceCollection==nbrMaxCollection){
        ...
        if(nrow(mydata)>0){
          if(mykeyA=='_row')
          {
            ...
            if(class(df)=="character"){
              ...
              #myNewConstrainte<-myConstraintUniqueIN(mykeyB,mydata[,mykeyA])
            }else if(class(df) %in% c("numeric","integer")){
        ...
        if(myNewConstrainte==F){
          ...
          if(nrow(globalInformation$modules$Catalogue$dataFrameContrainte)>0){
            for(k in 1:nrow(globalInformation$modules$Catalogue$dataFrameContrainte)){
              if(
                ...
                &&
                (globalInformation$modules$Catalogue$dataFrameContrainte[k,"id_collection"]==listeCollection[indiceCollection+1])
              ){
          ...
          globalInformation$modules$Catalogue$dataFrameContrainte[i+1,"id_collection"]<-listeCollection[indiceCollection+1]
          globalInformation$modules$Catalogue$dataFrameContrainte[i+1,"collection"]<-str_sub(listeCollection[indiceCollection+1],1,str_length(listeCollection[indiceCollection+1])-1)
      ...
      globalInformation$modules$Catalogue$dataFrameRequete[globalInformation$modules$Catalogue$dataFrameRequete["id_collection"]==lc,"nbrElemRequete"]<-nbrElemRequete
      output[[ERROR_MGS_ActionSave_data_frame_requete_Les_Genes]]<-renderText({
