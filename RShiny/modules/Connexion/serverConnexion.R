  ...
  function(input, output, session,globalInformation){
    ...
    observeEvent(input$ActionLogin,{
      ...
      if(nrow(dataUser)>0){
        ...
        hash <- toString(sapply(dataUser[1,"Password"], as.character))
        ...
        if( testPwd== TRUE) {
          ...
          if(dataUser[1,"connecter"]==1){
            if(nrow(app$usersTracking)>0){
              for(i in 1:nrow(app$usersTracking)){
                if(!is.null(app$usersTracking[i,"email"])){
                  if(app$usersTracking[i,"email"]==dataUser[1,"email"]){
                    trouver<-T
                  }
                }
              ...
              if(trouver==F){
                app$usersTracking <<- rbind(app$usersTracking,dataUser)
              }
            }else{
              app$usersTracking <<- rbind(app$usersTracking,dataUser)
            }
            ...
            col<-intersect(names(app$usersTracking),names(dataUser))
            # app$db <<- intersect(names(app$usersTracking),names(dataUser))
            # cat(length(col))
            app$usersTracking <<- rbind(app$usersTracking[,col],dataUser[,col])
          }
          ...
          globalInformation$user$id<-dataUser[1,"Name"]
          globalInformation$user$nom<-dataUser[1,"First_name"]
          globalInformation$user$prenom<-dataUser[1,"Last_name"]
          globalInformation$user$email<-dataUser[1,"email"]
          globalInformation$user$profil<-dataUser[1,"profil"]
          globalInformation$user$access_level<-dataUser[1,"access_level"]
          ...
          my_config_df <- app$config_df[which((as.integer(globalInformation$user$access_level) >= app$config_df$priority) & (!is.na(app$config_df$menu))),]
          # @ludo : (as.integer(globalInformation$user$access_level) +2h
          my_config_df <- my_config_df[order(my_config_df$rang),]
          # cat("\naccess_level = ",globalInformation$user$access_level,
          #     " email = ",globalInformation$user$email,
          #     " nbr module fr =",length(app$modules$menu),
          #     " nbr module my_config_df =",nrow(my_config_df),
          #     "all =",nrow(app$config_df))
          
          #my_config_df<-app$config_df
