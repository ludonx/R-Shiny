...
globalInformation$modules <- reactiveValues(
  ...
  # ** blast ----
  blast = reactiveValues(
    time = Sys.time(),
    save_session_time = 1,
    variables = list(),
    
    pathToModule = "/share/apps/modules/Modules/3.2.10/",
    pathToICG ="/data/db/biobank/catalogue/hs_9.9/original_files/",
    pathToSaveResult = paste0(app$config$workDirectory,"/")
  )
)
