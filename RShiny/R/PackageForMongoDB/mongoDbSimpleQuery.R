...
############################################################
addNewUser<-
  function(databaseName,collectionName,Name,First_name,Last_name,Password,email,profil,Projects="none",access_level=1){
  queryInsert<-
    paste0('{"Name" : "',Name,
...
deleteUser<-
  function(databaseName,collectionUserDelete,collectionUser,Name,First_name,Last_name,Password,email,profil,date_ajout,Projects="none"){
    ...
    db$remove(paste0('{"Name" : "',Name,'"}'), just_one = TRUE)
  }
############################################################
ModifierUser<-
  function(databaseName,collectionName,Name,First_name,Last_name,Password,email,profil,access_level){
    queryInsert<-
    ...
    db$update(paste0('{"Name":"',Name,'"}'), paste0('{"$set":',queryInsert,'}'))
    #db$update(paste0('{"email":"',email,'"}'), paste0('{"$set":',queryInsert,'}')) ici
    # Automatically disconnect when connection is removed
    # rm(db)
    # gc()
  }
...
###################################################################################################
###################################################################################################
typeColumnCollectionsDefinition<-
  function(){
    ...
    #dataFrameObservation<-findQuery(globalDatabase,'collections',myQueryUniqueIN("name","obs_mgs_cat_hs9.9_v1_Microbaria"))
    dbc<-""
    ...
    for(i in 1:nrow(dataFrameDefinition)){
      ...
      mydataCollection<-findLimit(dataFrameDefinition[i,"database"],collectionName,20)
      ...
      for(j in 1:length(listeColumn)){
        ...
        min<-max<-0
        if(typeColumn %in% c("integer","numeric")){
          
          dbc<-loadDataBase(dataFrameDefinition[i,"database"],collectionName)
        ...
        cat("\n>> Insert ok : --",j,"--",myqueryInsert)
      }
      
    }
    
  }
typeColumnCollectionsObservation<-
  function(collectionAbundance = "obs_mgs_cat_hs9.9_v1_Microbaria"){
    #### on crée un collection qui contiendra les type des differentes colonnes des définitions 
    ### cele permetre de mieux gére les interface des requêtes
    globalDatabase<-'iogold'
    #dataFrameDefinition<-findQuery(globalDatabase,'collections',myQueryChaine("type","def"))
    dataFrameObservation<-findQuery(globalDatabase,'collections',myQueryUniqueIN("name",collectionAbundance))
    dbc<-""
    db<-loadDataBase(globalDatabase,"typeColumn2")
    db$drop()
    # @ludo apres il faudra concatener cette collection à la collection typeColumn2
    
    for(i in 1:nrow(dataFrameObservation)){
      collectionName<-dataFrameObservation[i,"name"]
      
      mydataCollection<-findLimit(dataFrameObservation[i,"database"],collectionName,20)
      listeColumn<-colnames(mydataCollection)
      for(j in 1:length(listeColumn)){
        columnName<-listeColumn[j]
        typeColumn<-class(mydataCollection[,columnName])
        ...
        if(typeColumn %in% c("integer","numeric")){
          ...
          #dbc<-loadDataBase(dataFrameObservation[i,"database"],collectionName)
          #mm<-dbc$aggregate(paste0('[{ "$group" : { "_id": null, "max": { "$max" : "',paste0("$",columnName),'" }, "min": { "$min" : "',paste0("$",columnName),'" }}}]'))
...
executerQueriesDataFrame<-
  function(globalDatabase,jointuresCollections,listeQueries,dataFrameRequete){#,nameSaveDataFrame
    ...
    if(nrow(dataFrameRequete)>0){
      for(i in 1:nrow(dataFrameRequete) ){
        ...
        if(!is.null(listeQueries[i]))dataFrameRequete[i,"requete"]<-listeQueries[i]
        #cat(dataFrameRequete[i,"requete"])
        if(i==1){
          
          # #if(dataFrameRequete[i,"requete"]=='{}'){
          #   #cat("1 :",myQueryStringInit)
          #   dataFrameRequete[i,"requete"]<-listeQueries[i]
          # #}
...
executerQueriesDataFrameProjet<-
  function(globalDatabase,jointuresCollections,dataFrameInit,collectiondataFrameInit,dataFrameRequete,nameSaveDataFrame){
    ...
    newRequete[1,"id_collection"]<-collectiondataFrameInit
