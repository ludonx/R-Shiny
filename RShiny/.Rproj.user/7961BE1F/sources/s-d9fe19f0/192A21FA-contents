    ...
    observe({
      ...
      if((diff_time)>globalInformation$user$save_session_time){
        ...
        if(globalInformation$user$connected==T){
          ...
          #cat("ok\n",length(globalInformation$modules))
          name_module <- names(globalInformation$modules)
          #for( i in 1:length(name_module)){cat("\n-",name_module[i])}
          #isolate(gi$modules <- reactiveValuesToList(globalInformation$modules[name_module])) # erreur : il faut deux crochet
          gi$modules <- list()
          name_module <- names(globalInformation$modules)
          for( i in 1:length(name_module))
          {
            nmod <- name_module[i] 
            
            #gi$modules[[nmod]] <- reactiveValuesToList(globalInformation$modules[[nmod]]) # ceci ne fonctionne pas, je ne sais pas pourquoi
            x<-reactiveValuesToList(globalInformation$modules[[nmod]])
            gi$modules[[nmod]] <- x
          }
          ...
          # {
          #   nmod <- name_module[i] 
          #   #isolate(gi$modules[nmod] <- reactiveValuesToList(globalInformation$modules[[nmod]]))
          #   gi$modules <- reactiveValuesToList(globalInformation$modules[[nmod]])
          # }
          # 
          #printy(names(reactiveValuesToList(globalInformation$modules[["blast"]])))
          ...
          #   for( i in 1:length(globalInformation$modules)){gi$modules[names(globalInformation$modules)[i]] <- reactiveValuesToList(globalInformation$modules[i])},
          #   
