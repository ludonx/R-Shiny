...
serverLes_Genes<-
  function(input, output, session,globalInformation){
    ...
    output$uiInfo_catalog_Les_Genes<-renderText({
      ...
      #   }
      # }
      globalInformation$modules$Catalogue$CatalogSelected<-input$select_catalog_Les_Genes
      
      mydataFrameCatalog_row_selected<-dataFrameCatalog()[dataFrameCatalog()["name"]==globalInformation$modules$Catalogue$CatalogSelected,]
    ...
    dataFrameDefinition<-reactive({
      ...
      ## on selectionne les definitions du catalogue 
      dataFrameDefinition_def<-
        findQuery(
      ...
      ## on selectionne les observations des projets de cette utilisateur
      dataProjectUser<-
        getProjectUser(
          globalInformation$globalDatabase,
          ...
          input$select_catalog_Les_Genes
        )
      dataFrameDefinition_obs <- data.frame()
      if(nrow(dataProjectUser)>0){
        dataFrameDefinition_obs<-
          findQuery(
            globalInformation$globalDatabase,
            globalInformation$collections,
            myQueryConstraintN(
              list(
                myConstraintUniqueIN("project",dataProjectUser[,'name']),
      ...
      dataFrameDefinition<-dataFrameDefinition_def
      dataFrameDefinition<-rbind(dataFrameDefinition,dataFrameDefinition_obs)
    ...
    observe({
      ...
      if(nrow(dataFrameDefinition())>0){
        ...
        df$duplicate <- 1
        isolate(globalInformation$modules$Catalogue$dataFrameDefinition <- df[,c('name','duplicate','project')])
        
      }
    })
    output$data_frame_select_def_catalog_Les_Genes<- DT::renderDataTable({
      req(input$select_catalog_Les_Genes)
      choices=c()
      
      DT::datatable(globalInformation$modules$Catalogue$dataFrameDefinition,
                    selection = 'multiple',
                    width ="2px",
                    # editable = list(
                    #   target = 'row',
                    ...
                    options = list(
                      #autoWidth = TRUE,
                      #columnDefs = list(list(width = '5px', targets = c(1,2))),
                      pageLength = 20,
                      lengthMenu = c(20, 40, 80, 160)
                    ))
      
    })
    #### Config pour gerer les modification du data frame

    proxy_data_frame_select_def_catalog_Les_Genes = dataTableProxy('data_frame_select_def_catalog_Les_Genes')

    observeEvent(input$data_frame_select_def_catalog_Les_Genes_cell_edit, {
      info = input$data_frame_select_def_catalog_Les_Genes_cell_edit
      #str(info)
      i = info$row
      j = info$col
      v = info$value
      globalInformation$modules$Catalogue$dataFrameDefinition[i, j] <<- DT::coerceValue(v, globalInformation$modules$Catalogue$dataFrameDefinition[i, j])
      replaceData(proxy_data_frame_select_def_catalog_Les_Genes, globalInformation$modules$Catalogue$dataFrameDefinition, resetPaging = FALSE)  # important
    })
    ### on selection une ou plusieurs definition
    output$uiselect_def_catalog_Les_Genes<-renderUI({
      req(input$select_catalog_Les_Genes)
      
      lineSelected <- input$data_frame_select_def_catalog_Les_Genes_rows_selected
      choices <- c()
      choicesName <- c()
      myselect <- list()
      df<-globalInformation$modules$Catalogue$dataFrameDefinition
      nbMax<-length(lineSelected)
      if(nbMax){
        # choices <- lapply(1:nbMax, function(i){
        #     for(j in 1:df[lineSelected[i],"duplicate"]){
        #       return(df[lineSelected[i],"name"])
        #       }
        #     })
        for( i in 1:nbMax)
        {
          for(j in 1:df[lineSelected[i],"duplicate"]){
            x<-paste0(df[lineSelected[i],"name"],j)
            choices <- append(choices,x)
            choicesName<-append(choicesName,x)
            #choicesName <- append(choicesName,str_sub(x,1,str_length(x)-1))
      ...
      t = setNames(choices,choicesName)
      selected<-choices
    ...
    output$uiguide_select_def_catalog_Les_Genes<-renderText({
      ...
      nbr<-length(listDef)
      last_def<-str_sub(listDef[nbr],1,str_length(listDef[nbr])-1)
      # Create a Progress object
      progress <- shiny::Progress$new()
      # Make sure it closes when we exit this reactive, even if there's an error
      on.exit(progress$close())
    ...
    testOrdre<-eventReactive(input$ActionValider_definition_Les_Genes,{
      ...
      globalInformation$modules$Catalogue$defSelected<-list()
      globalInformation$modules$Catalogue$databaseDefSelected<-list()
      ...
      defSelected<-list()
      for( indice in 1:length( globalInformation$modules$Catalogue$defSelected)){
        lc<-globalInformation$modules$Catalogue$defSelected[indice]
        lc<-str_sub(lc,1,str_length(lc)-1)
        defSelected<-lc
        globalInformation$modules$Catalogue$databaseDefSelected <- 
          append(globalInformation$modules$Catalogue$databaseDefSelected,
                 as.list(
                   dataFrameDefinition()[
                     dataFrameDefinition()["name"]==lc ,
                     "database"]
                 )
          )
      }
