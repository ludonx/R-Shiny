...
serverblast<-
  function(input, output, session,globalInformation){
    ...
    observeEvent(input$valider_blast,{
      req(input$blast_output_file)
      req(input$input$evalue_blast)
      if(class(input$evalue_blast)=='character'){
        output$res_blast<-renderText({paste0("-evalue doit être un entier")})
